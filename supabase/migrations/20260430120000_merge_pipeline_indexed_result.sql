-- Atomic merge for parallel Fal webhooks: one indexed slot per transaction, row-locked.
-- became_complete is true only when this call raised filled_count from < expected to == expected.
...
DECLARE
  ...
  v_elem jsonb;
BEGIN
  IF p_results_key NOT IN ('patch_results', 'brass_results', 'background_results') THEN
