-- Fix: the deployed merge_pipeline_indexed_result counted EMPTY STRINGS as
-- filled slots (jsonb_array_elements_text ... WHERE t IS NOT NULL AND t <> 'null').
-- Since submitFinalEditStage pre-initializes final_edit_results to ["","","",""],
-- every call returned filled_count = 4 and became_complete = false, so the
-- judge/delivery transition never fired after the webhook switched to
-- became_complete-only triggering. A slot now counts as filled ONLY when it is
